    ...
    def canPartitionKSubsets(self, nums: List[int], k: int) -> bool:
        ...
        def backtrack(i, k, curr_sum):
            ...
            for j in range(i, len(nums)):
                ...
                if visited[j] or curr_sum + nums[j] > global_target or (j and not visited[j-1] and nums[j] == nums[j-1]):
                    continue
                ...
                # if curr_sum is 0 and we weren't able to find a success, we won't be able to find
                # any success with this current state because we would just be rearranging the
                # remaining subsets.
                if curr_sum == 0:
                    break
            return False
